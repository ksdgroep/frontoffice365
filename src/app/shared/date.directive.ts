...
export class DateDirective implements Validator, ControlValueAccessor {
  ...
  writeValue(obj: any): void {
    if (obj) {
      const textDate = this.datePipe.transform(obj, 'dd-MM-yyyy');
  ...
  validate(c: AbstractControl): { [key: string]: any; } {
    ...
    const values = this.testDate.split('-');
    const date = new Date(parseInt(values[2], 10), parseInt(values[1], 10) - 1, parseInt(values[0], 10));
    ...
    if ((date.getFullYear() !== parseInt(values[2], 10)) || (date.getMonth() !== parseInt(values[1], 10) - 1)) {
      return {validDate: false};
    }
  ...
  @HostListener('input', ['$event.target.value']) onChange = (value: any) => {
    ...
    const date = new Date(Date.UTC(values[2], values[1] - 1, values[0]));
